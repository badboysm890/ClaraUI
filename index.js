...
function renderChatBox(chatData) {
    ...
    chatData.forEach(chat => {
        ...
        // if there is link to a image, then render it as an image
        chat.content = formatImageLinks(chat.content);

        chat.content = formatLinks(chat.content)

        let newChat = document.createElement('div');
        newChat.classList.add(chat.role === 'assistant' ? 'bot-chat' : 'user-chat');

        let innerDiv = document.createElement('div');
        innerDiv.classList.add('d-inline-block', chat.role === 'assistant' ? 'float-left' : 'float-right', 'mb-1');
        innerDiv.style.maxWidth = '50%';

        let pText = document.createElement('p');
        pText.classList.add(chat.role === 'assistant' ? 'bg-danger' : 'bg-primary', 'text-white', 'py-2', 'px-3', 'rounded', 'text-break');
...
function sendChat(chatInput) {
    ...
    let chatData = { 'chatInput': chatInput, 'chatId': localStorage.getItem('userid') }
        // if fileModeEnabled is true, then send the file name instead of the chatInput
    if (localStorage.getItem('fileModeEnabled') == "true") {
        chatData = { 'chatInput': chatInput, 'chatId': localStorage.getItem('userid'), 'fileModeEnabled': true, 'fileModeType': localStorage.getItem('fileModeType') }
    }

    $.ajax({
        type: "POST",
        url: "http://127.0.0.1:5000/send_chat",
        data: chatData,
        success: function(response) {
            response.response.splice(0, 3);
            renderChatBox(response.response);
